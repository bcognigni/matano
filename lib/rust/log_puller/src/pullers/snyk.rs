    ...
    async fn pull_logs(
        ...
        if tables_config.get("vulnerabilities").is_some() {
            ...
            loop {
                ...
                let url = format!(
                    "https://api.snyk.io/v1/reporting/issues/?from={}&to={}&page={}&perPage=100&sortBy=issueTitle&order=asc&groupBy=issue",
                    start_day,
                    yesterday,
                    page
                );
                ...
                let response_json: Vec<serde_json::Value> = response.json().await?;
                info!("This is the response from reporting: {}", &response_json);
                let length = response_json.len();
